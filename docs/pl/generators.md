...
Generuje spójne imię, nazwisko, pełną nazwę, płeć i e-mail. Używaj przez grupę;
można mapować dowolny niepusty podzbiór wyjść `FirstName`, `LastName`, `FullName`,
`Gender`, `Email`.
...
- `FullNamePattern`: `FirstNameLastName` albo `LastNameFirstName`.
